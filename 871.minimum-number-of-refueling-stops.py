...
class Solution:
    def minRefuelStops(self, target: int, startFuel: int, stations: List[List[int]]) -> int:
        ans, i, n = 0, 0, len(stations)
        pos, max_pos = 0, startFuel
        ...
        pq = []
        while True:
            # NOTE add the fuel of the passed stations to the pq
            while i < n and stations[i][0] <= max_pos:
                heapq.heappush(pq, -stations[i][1])
                i += 1

            if pq:
                # NOTE select the max fuel to "refuel" (to extend the max reachable distance)
                neg_max_fuel = heapq.heappop(pq)
                max_pos -= neg_max_fuel
                ans += 1
                if max_pos >= target:
                    return ans
            else:
