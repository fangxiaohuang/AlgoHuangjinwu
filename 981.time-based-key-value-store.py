...
class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
        if key not in self.map:
            self.map[key] = [[timestamp], {timestamp: value}]
        else:
            self.map[key][0].append(timestamp)
            self.map[key][1][timestamp] = value


    def get(self, key: str, timestamp: int) -> str:
        if key not in self.map:
            return ""
        timestamps, ts_value_dict = self.map[key]
        no = bisect.bisect(timestamps, timestamp)
        if no == 0: # Not found
            return ""
        return ts_value_dict[timestamps[no-1]]
